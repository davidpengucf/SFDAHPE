...
from lib.models.loss import JointsMSELoss, ConsLoss, SimCCLoss, ResLoss, CstLoss
...
def main(args: argparse.Namespace):
    ...
    train_target_dataset = target_dataset(root=args.target_root, split='all', transforms_base=base_transform,
    ...
    val_target_dataset = target_dataset(root=args.target_root, split='all', transforms=val_transform,
                                        image_size=image_size, heatmap_size=heatmap_size)
    ...
    # create model
    model_sr = models.__dict__[args.arch](num_keypoints=train_source_dataset.num_keypoints).cuda()
    model_tg = models.__dict__[args.arch](num_keypoints=train_source_dataset.num_keypoints).cuda()
    model_in = models.__dict__[args.arch](num_keypoints=train_source_dataset.num_keypoints).cuda()

    #print(model_sr.backbone)
    #print(model_sr.parameters)
    #print(model_sr.backbone.parameters)
    
    criterion = JointsMSELoss()
    con_criterion = ConsLoss()
    res_criterion = ResLoss()
    cst_criterion = CstLoss()
    im_criterion = SimCCLoss()

    print('student optimizer is SGD? ', args.SGD)

    if args.SGD:
        pre_optimizer = SGD(model_sr.parameters(), lr=args.lr, momentum=0.9, weight_decay=0.0001, nesterov=True)
        tg_optimizer = SGD(model_tg.parameters(), lr=args.lr, momentum=0.9, weight_decay=0.0001, nesterov=True)

        sr_optimizer = SGD(list(model_sr.upsampling.parameters()) + list(model_sr.head.parameters()), lr=args.lr, momentum=0.9, weight_decay=0.0001, nesterov=True)
        in_optimizer = SGD(model_in.backbone.parameters(), lr=args.lr*0.1, momentum=0.9, weight_decay=0.0001, nesterov=True)
    else:
        pre_optimizer = Adam(model_sr.parameters(), lr=args.lr)
        tg_optimizer = Adam(model_tg.parameters(), lr=args.lr)

        sr_optimizer = Adam(list(model_sr.upsampling.parameters()) + list(model_sr.head.parameters()), lr=args.lr)
        in_optimizer = Adam(model_in.backbone.parameters(), lr=args.lr*0.1)

    ema_optimizer = OldWeightEMA(model_in, model_tg, alpha=args.teacher_alpha)

    lr_scheduler = MultiStepLR(tg_optimizer, args.lr_step, args.lr_factor)
    ...
    if args.resume:
        ...
        model_sr.load_state_dict(checkpoint['sr'])
        model_tg.load_state_dict(checkpoint['tg'])
        model_in.load_state_dict(checkpoint['in'])
        pre_optimizer.load_state_dict(checkpoint['pre_optimizer'])
        tg_optimizer.load_state_dict(checkpoint['tg_optimizer'])
        sr_optimizer.load_state_dict(checkpoint['sr_optimizer'])
        in_optimizer.load_state_dict(checkpoint['in_optimizer'])
        # tea_optimizer.load_state_dict(checkpoint['tea_optimizer'])
        lr_scheduler.load_state_dict(checkpoint['lr_scheduler'])
        start_epoch = checkpoint['epoch'] + 1

    elif args.pretrain:
        pretrained_dict = torch.load(args.pretrain, map_location='cpu')['sr']
        model_dict = model_tg.state_dict()
        # remove keys from pretrained dict that doesn't appear in model dict
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
        model_sr.load_state_dict(pretrained_dict, strict=False)
        model_tg.load_state_dict(pretrained_dict, strict=False)
        model_in.load_state_dict(pretrained_dict, strict=False)
    ...
    if args.phase == 'test':
        ...
        source_val_acc = validate(val_source_loader, model_in, criterion, None, args)
        target_val_acc = validate(val_target_loader, model_in, criterion, visualize, args)
    ...
    for epoch in range(start_epoch, args.epochs): # start_epoch ~ 70
        logger.set_epoch(epoch)
        lr_scheduler.step()

        # train for one epoch
        if epoch < args.pretrain_epoch: # 40
            pretrain(train_source_iter, model_sr, criterion, pre_optimizer, epoch, visualize if args.debug else None, args)
        else:
            if epoch == args.pretrain_epoch:
                pretrained_dict = torch.load(logger.get_checkpoint_path('best_pt'), map_location='cpu')['sr']
                model_dict = model_tg.state_dict()
                pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
                model_sr.load_state_dict(pretrained_dict, strict=False)
                model_tg.load_state_dict(pretrained_dict, strict=False)
            ...
            train(train_target_iter, model_sr, model_tg, model_in, criterion, con_criterion, res_criterion, cst_criterion, im_criterion, 
                    tg_optimizer, sr_optimizer, in_optimizer, ema_optimizer, epoch, args)

        # evaluate on validation set
        if epoch < args.pretrain_epoch: # pretrain: validate on student model
            source_val_acc = validate(val_source_loader, model_tg, criterion, None, args)
            target_val_acc = validate(val_target_loader, model_tg, criterion, visualize if args.debug else None, args)
        else: # validate on teacher model
            source_val_acc = validate(val_source_loader, model_in, criterion, None, args)
            target_val_acc = validate(val_target_loader, model_in, criterion, visualize if args.debug else None, args)

        if target_val_acc['all'] > best_acc:
            torch.save(
                {
                    'sr': model_sr.state_dict(),
                    'tg': model_tg.state_dict(),
                    'in': model_in.state_dict(),
                    'pre_optimizer': pre_optimizer.state_dict(),
                    'sr_optimizer': sr_optimizer.state_dict(),
                    'tg_optimizer': tg_optimizer.state_dict(),
                    'in_optimizer': in_optimizer.state_dict(),
...
def train(train_target_iter, model_sr, model_in, model_tg, criterion, con_criterion, res_criterion, cst_criterion, im_criterion,
          tg_optimizer, sr_optimizer, in_optimizer, ema_optimizer, epoch: int, args: argparse.Namespace):
    batch_time = AverageMeter('Time', ':4.2f')
    data_time = AverageMeter('Data', ':3.1f')
    losses_ft = AverageMeter('Loss (ft)', ":.4e")
    losses_tg = AverageMeter('Loss (tg)', ":.4e")
    losses_in = AverageMeter('Loss (in)', ":.4e")
    acc_s = AverageMeter("Acc (s)", ":3.2f")

    progress = ProgressMeter(
        args.iters_per_epoch,
        [batch_time, data_time, losses_ft, losses_tg, losses_in, acc_s],
        prefix="Epoch: [{}]".format(epoch))
    ...
    scaler_sr = torch.cuda.amp.GradScaler()
    scaler_in = torch.cuda.amp.GradScaler()
    scaler_tg = torch.cuda.amp.GradScaler()
    ...
    for i in range(args.iters_per_epoch):
        ...
        with torch.no_grad():
            y_t_teas = [model_in(x_t_tea) for x_t_tea in x_t_teas] # softmax on w, h
        ...
        with torch.cuda.amp.autocast():
            ...
            y_t_in = model_in(x_t_stu)
            y_t_tg = model_tg(x_t_stu) # softmax on w, h

            #loss_c = con_criterion(y_t_stu_recon, y_t_tea_recon, tea_mask=tea_mask)
            loss_con = criterion(y_t_in, y_t_tg)
            loss_cst = cst_criterion(y_t_in, y_t_tg)
            loss_im = im_criterion(y_t_in, y_t_tg)
            loss_tg = loss_con + 0.5 * loss_cst + 0.85 * loss_im



        scaler_tg.scale(loss_tg).backward()
        scaler_tg.step(tg_optimizer)
        ema_optimizer.step()

        scaler_tg.update()

        # measure accuracy and record loss
        losses_ft.update(loss_ft, x_t_stu.size(0))
        losses_tg.update(loss_tg, x_t_stu.size(0))
        losses_in.update(loss_in, x_t_stu.size(0))
