...
class LSP_mt_con(Body16KeypointDataset):
    ...
    def __init__(self, root, split='train', task='all', download=True, image_size=(256, 256), k=1, 
        ...
        super(LSP_mt_con, self).__init__(root, samples, image_size=image_size, **kwargs)
    ...
    def __getitem__(self, index):
        ...
        image_stu_1, data_stu_1 = self.transforms_stu(image, keypoint2d=keypoint2d, intrinsic_matrix=None)
        keypoint2d_stu_1 = data_stu_1['keypoint2d']
        aug_param_stu_1 = data_stu_1['aug_param']

        # 2D heatmap
        target_stu_1, target_weight_stu_1 = generate_target(keypoint2d_stu_1, visible, self.heatmap_size, self.sigma, self.image_size)
        target_stu_1 = torch.from_numpy(target_stu_1)
        target_weight_stu_1 = torch.from_numpy(target_weight_stu_1)

        meta_stu_1 = {
            'image': image_name,
            'target_small_stu': generate_target(keypoint2d_stu_1, visible, (8, 8), self.sigma, self.image_size),
            'keypoint2d_ori': keypoint2d,  
            'target_ori': target_ori,  
            'target_weight_ori': target_weight_ori,  
            'keypoint2d_stu': keypoint2d_stu_1,  # （NUM_KEYPOINTS x 2）
            'aug_param_stu': aug_param_stu_1,
        }

        image_stu_2, data_stu_2 = self.transforms_stu(image, keypoint2d=keypoint2d, intrinsic_matrix=None)
        keypoint2d_stu_2 = data_stu_2['keypoint2d']
        aug_param_stu_2 = data_stu_2['aug_param']

        # 2D heatmap
        target_stu_2, target_weight_stu_2 = generate_target(keypoint2d_stu_2, visible, self.heatmap_size, self.sigma, self.image_size)
        target_stu_2 = torch.from_numpy(target_stu_2)
        target_weight_stu_2 = torch.from_numpy(target_weight_stu_2)
        ...
        meta_stu_2 = {
            ...
            'target_ori': target_ori,  
            'target_weight_ori': target_weight_ori,  
            'keypoint2d_stu': keypoint2d_stu_2,  # （NUM_KEYPOINTS x 2）
            'aug_param_stu': aug_param_stu_2,
        }
        ...
        return image_stu_1, target_stu_1, target_weight_stu_1, meta_stu_1, image_stu_2, target_stu_2, target_weight_stu_2, meta_stu_2, images_tea, targets_tea, target_weights_tea, metas_tea
